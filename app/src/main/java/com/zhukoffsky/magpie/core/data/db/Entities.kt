...
data class ReminderEntity(
    ...
    val syncState: SyncState = SyncState.LOCAL_ONLY,
    /**
     * Срок отложенного показа; `null` — отсрочки нет.
     *
     * Хранится ради перезагрузки. Раньше отсрочка была только будильником, и
     * выключенный телефон её терял: `rescheduleAll` знает лишь `dueAt`, а
     * отложенное срабатывание к нему отношения не имеет. Для повтора это
     * значило «вернётся завтра», для одноразового — «не вернётся никогда».
     */
...
data class MedIntakeEntity(
    ...
    /**
     * Срок отложенного напоминания о приёме; `null` — отсрочки нет.
     *
     * Статус `SNOOZED` говорил только о факте отсрочки, но не о её сроке,
     * поэтому пережить перезагрузку она не могла. Причина та же, что и у
     * напоминаний, и цена выше: пропущенная доза — это не забытая покупка.
     */
    val snoozedUntil: Instant? = null,
)
