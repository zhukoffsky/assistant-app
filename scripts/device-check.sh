...
run() { printf '$ %s\n' "$*" >>"$REPORT"; "$@" >>"$REPORT" 2>&1; }
pause() {
    printf '\n>>> %s\n    Сделай это на телефоне и нажми Enter... ' "$*"
    # Без терминала `read` возвращается сразу по EOF. Скрипт прошёл бы все
    # шаги за секунду и написал отчёт, в котором ни одна ручная проверка не
    # выполнялась, — а выглядел бы он как настоящий.
    read -r _ || { printf '\nСтандартный ввод закончился. Отчёт неполный.\n'; exit 1; }
}
